...
CREATE POLICY "Driver can view their own device" 
ON public.driver_devices 
FOR SELECT 
USING (true); -- Per ora aperto, poi si può limitare per utente

CREATE POLICY "Driver can update their own device" 
ON public.driver_devices 
FOR UPDATE 
USING (true);

CREATE POLICY "Driver can insert their own device" 
ON public.driver_devices 
FOR INSERT 
WITH CHECK (true);

-- Politiche RLS per gps_tracking  
CREATE POLICY "Anyone can view GPS tracking" 
ON public.gps_tracking 
FOR SELECT 
USING (true);

CREATE POLICY "Driver can insert GPS data" 
ON public.gps_tracking 
FOR INSERT 
WITH CHECK (true);
